...
export class RegistrationService {
  // Seat reservation uses atomic RPC: UPDATE ... WHERE seats_remaining > 0
  // wrapped in same Postgres transaction as registration INSERT + notification outbox.
  ...
  async getMyRegistrations(mssv: string): Promise<RegistrationRow[]> {
    ...
    return Promise.all(
      rows.map(async r => ({
        ...r,
        qr_image: r.qr_token ? await QRCode.toDataURL(r.qr_token) : undefined,
      })),
    )
  }

  async getAllRegistrations(opts: GetAllRegistrationsOptions = {}): Promise<AdminRegistrationRow[]> {
    let query = supabase
      .from('registrations')
      .select(`
        id, mssv, workshop_id, status, qr_token,
  ...
  // Called by cron every 60s — atomic via RPC
  async releasePendingSeats(): Promise<void> {
...
function throwRegistrationError(error: { code?: string; message?: string } | null): never {
  ...
  if (message.includes('STUDENT_NOT_VERIFIED')) {
    throw Object.assign(
      new Error('MSSV not found in student whitelist or has been deactivated'),
  ...
  if (code === '23505' || code === '23P01' || message.includes('registrations_unique_active')) {
