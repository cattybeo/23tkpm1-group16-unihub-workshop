...
```mermaid
sequenceDiagram
    participant Cam as PWA Scanner (Camera)
...
```mermaid
sequenceDiagram
    participant Cam as PWA Scanner
...
2. **Quyền truy cập (RBAC):** Chỉ user có role `scanner` hoặc `organizer` mới được gọi các API `POST /api/v1/check-ins` và `/sync`. Sinh viên không tự check-in được (tham chiếu spec `access-control.md`).
